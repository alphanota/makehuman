...
class RandomizeAction(gui3d.Action):
    def __init__(self, human, before, after):
        super(RandomizeAction, self).__init__("Randomizer")
        self.human = human
        self.before = before
        self.after = after
...
class RandomTaskView(gui3d.TaskView):
    ...
    def __init__(self, category):
        gui3d.TaskView.__init__(self, category, 'Random save')
        ...
        self.reps = toolbox.addWidget(gui.TextEdit("1000", validator=QIntValidator()))
...
def randomize(human, symmetry, macro, height, face, reps, body):
    modifiers = get_modifier_groups(body, face, height, human, macro)
    df = pd.DataFrame(columns=modifiers)
    for r in range(0, reps):
        randomValues = get_random_values(human, modifiers, symmetry)
        new_row = pd.DataFrame(randomValues,index=[0])
        df = df.append(new_row,ignore_index=True)
        oldValues = dict( [(m.fullName, m.getValue()) for m in modifiers] )
        gui3d.app.do( RandomizeAction(human, oldValues, randomValues) )
        filename = '/Users/angel/Documents/render/' + 'V_' + str(r) + '.png'
        human = G.app.selectedHuman
        human.applyAllTargets()
        mh.redraw()
        mh.grabScreen(1, 1, G.windowWidth - 3, G.windowHeight - 3, filename)
        # print(filename)
        #gui3d.app.getCategory('Rendering').getTaskByName('Scene').image.save(filename)

    df.to_csv('/Users/angel/mhhm_dataset.csv')

def get_random_values(human, modifiers, symmetry):
    randomValues = {}
    for m in modifiers:
        if m.fullName not in randomValues:
            ...
            elif m.groupName in ["head", "forehead", "eyebrows", "neck", "eyes", "nose", "ears", "chin", "cheek",
                                 "mouth"]:
                sigma = 0.1
            ...
            if randomValue is None:
                randomValue = getRandomValue(m.getMin(), m.getMax(), m.getDefaultValue(),
                                             sigma)  # TODO also allow it to continue from current value?
...
def get_modifier_groups(body, face, height, human, macro):
    modifierGroups = []
    if macro:
